...
RegisterNetEvent('qb-vehicleshop:client:openCustomFinance', function(data)
    ...
    if dialog then
        if not dialog.downPayment or not dialog.paymentAmount or not dialog.citizenid then return end
        TriggerEvent('animations:client:EmoteCommandStart', {"c"})
        TriggerServerEvent('qb-vehicleshop:server:sellfinanceVehicle', dialog.downPayment, dialog.paymentAmount, data.vehicle, dialog.citizenid)
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:openIdMenu', function(data)
    ...
    if dialog then
        if not dialog.citizenid then return end
        if data.type == 'testDrive' then
            TriggerServerEvent('qb-vehicleshop:server:customTestDrive', data.vehicle, dialog.citizenid)
        elseif data.type == 'sellVehicle' then
            TriggerServerEvent('qb-vehicleshop:server:sellShowroomVehicle', data.vehicle, dialog.citizenid)
        end
    end
end)
