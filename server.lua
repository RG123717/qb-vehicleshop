...
AddEventHandler('playerDropped', function()
    ...
    if steam then
        local vehicles = MySQL.query.await('SELECT * FROM player_vehicles WHERE steam = ?', {steam})
        if vehicles then
...
RegisterNetEvent('qb-vehicleshop:server:buyShowroomVehicle', function(vehicle)
    ...
    if cash > tonumber(vehiclePrice) then
        MySQL.insert('INSERT INTO player_vehicles (steam, citizenid, vehicle, hash, mods, plate, garage, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
...
RegisterNetEvent('qb-vehicleshop:server:financeVehicle', function(downPayment, paymentAmount, vehicle)
    ...
    if cash >= downPayment then
        MySQL.insert('INSERT INTO player_vehicles (steam, citizenid, vehicle, hash, mods, plate, garage, state, balance, paymentamount, paymentsleft, financetime) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', {
            pData.PlayerData.steam,
            cid,
            vehicle,
            GetHashKey(vehicle),
...
RegisterNetEvent('qb-vehicleshop:server:sellShowroomVehicle', function(data, citizenid)
    local src = source
    local cid = target.PlayerData.citizenid
    local player = QBCore.Functions.GetPlayer(src)
    local target = QBCore.Functions.GetPlayer(cid)
...
QBCore.Commands.Add('transfervehicle', Lang:t('general.command_transfervehicle'), {{name = 'ID', help = Lang:t('general.command_transfervehicle_help')}, {name = 'amount', help = Lang:t('general.command_transfervehicle_amount')}}, false, function(source, args)
    ...
    local targetsteam = QBCore.Functions.GetIdentifier(target.PlayerData.source, 'steam')
    if not target then return TriggerClientEvent('QBCore:Notify', src, Lang:t('error.buyerinfo'), 'error') end
    if not sellAmount then
        MySQL.update('UPDATE player_vehicles SET citizenid = ?, steam = ? WHERE plate = ?', {targetcid, targetsteam, plate})
